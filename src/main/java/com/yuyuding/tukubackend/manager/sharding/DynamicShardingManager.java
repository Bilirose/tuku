package com.yuyuding.tukubackend.manager.sharding;
...
public class DynamicShardingManager {
    ...
    private void updateShardingTableNodes() {
        ...
        String newActualDataNodes = tableNames.stream()
                .map(tableName -> "tuku." + tableName)
                .collect(Collectors.joining(","));
        log.info("动态分表 actual-data-nodes: {}", newActualDataNodes);
    ...
    public void createSpacePictureTable(Space space){
        ...
        if(space.getSpaceType().equals(SpaceTypeEnum.TEAM.getValue()) && space.getSpaceLevel().equals(SpaceLevelEnum.FLAGSHIP.getValue())){
            ...
            try{
                ...
                e.printStackTrace();
                log.error("创建空间图片分表失败，空间 id = {}", spaceId   );
            }
        }
